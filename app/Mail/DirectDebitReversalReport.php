...
class DirectDebitReversalReport extends Mailable
{
    use Queueable;
    ...
    public function __construct($payments, Carbon $timestamp, Carbon $since, $files)
    {
        ...
        $this->subject = '[' . config('app.env') . '] Overzicht storno\'s ' . $timestamp->format('d-m-Y');
    ...
    {
        return $this->view('emails.reports.dd_reversals');
    }
}
